...
```mermaid
graph TD
    subgraph Z1["Zone 1 — SEC perimeter (SEC + PCF + UK GDPR)"]
        DCC["DCC\nInfrastructure controller"]
        SUP["Suppliers\nBilling + consent controllers"]
        DNO["DNOs / DSOs\nNetwork controllers"]
        OU["Other Users\nAccredited third parties"]
        INT["Intermediaries\nDCC service agents / CAD parties"]
    end

    subgraph Z2["Zone 2 — Contractual extension (MDA + UK GDPR)"]
        APP["App providers / ESCOs\nFlexibility aggregators"]
        SUB["Sub-processors\nAnalytics / cloud / API platforms"]
        HAN["HAN / CAD recipients\nDevice OEMs / HEMS vendors"]
    end

    subgraph Z3["Zone 3 — Beyond sector reach (UK GDPR only)"]
        IND["Independent controllers\nOwn-purpose processing"]
        ANO["Anonymised datasets\nOutside UK GDPR scope"]
        DER["Derived / transformed data\nFresh lawful basis required"]
    end

    DCC -->|"MDA required"| APP
    SUP -->|"MDA required"| APP
    SUP -->|"MDA required"| SUB
    OU  -->|"MDA required"| APP
    OU  -->|"MDA required"| SUB
    INT -->|"MDA required"| HAN

    APP -->|"Independent controller\nor anonymisation"| IND
    APP -->|"Independent controller\nor anonymisation"| ANO
    SUB -->|"Derived dataset —\nnew lawful basis"| DER
    HAN -->|"Independent controller\nor anonymisation"| IND

    style Z1 fill:#f3f0ff,stroke:#7c6fcf,stroke-width:1.5px,color:#3c2f8a
    style Z2 fill:#e8f8f2,stroke:#1d9e75,stroke-width:1.5px,color:#085041
    style Z3 fill:#f1efea,stroke:#888780,stroke-width:1.5px,color:#2c2c2a
```
...
## Notes on the multi-controller problem
...
*Framework version 1.1 — for consultation and modification context use*
