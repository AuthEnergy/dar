...
graph TD
    subgraph Z1["Zone 1 — SEC perimeter · SEC + PCF + UK GDPR"]
        ...
        B["e.g. Octopus, National Grid, n3rgy"]
    end

    subgraph Z2["Zone 2 — Contractual extension · MDA + UK GDPR"]
        C["App providers · Aggregators · Sub-processors"]
        D["e.g. Hildebrand, Mixergy, Tesla Energy"]
    end

    subgraph Z3["Zone 3 — Beyond sector reach · UK GDPR only"]
        E["Independent controllers · Anonymised data · Derived datasets"]
        F["e.g. price comparison sites, research bodies, ad-tech"]
    end

    Z1 -->|"MDA required\nbefore disclosure"| Z2
    Z2 -->|"GDPR only\nno sector reach"| Z3
